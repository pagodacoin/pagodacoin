...
#if defined(NDEBUG)
# error "Pagodacoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x127e9835de74d28498f3a6fee29dd86d76f5000b6a94c3a11cfe1f78271f73d4");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Pagodacoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Pagodacoin Signed Message:\n";
...
{
    // Pagodacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 100000); // Pagodacoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Pagodacoin: 1 days
static const int64 nTargetSpacing = 1 * 60; // Pagodacoin: 1 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Pagodacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Pagodacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x7c799500f6ededce04775159066a4a4658698e48a34b72bbc873323ebc46ce4a");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Pagoda Coin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1514643976;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084748282;

        if (fTestNet)
        {
            block.nTime    = 1514643931;
            block.nNonce   = 386146750;
        }
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xf935d12d8d6bec523a539f2bb0630752b6202ab94cff6ff977c583f34f39684a"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf9, 0xbe, 0xb7, 0xdc }; // Pagodacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("PagodacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("PagodacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static PagodacoinMiner(CWallet *pwallet)
{
    printf("PagodacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("pagodacoin-miner");
    ...
    try { loop {
        ...
        printf("Running PagodacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("PagodacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&PagodacoinMiner, pwallet));
}
